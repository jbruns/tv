...
| `unobservable` | A verification failure (it increments the failure count, same as `mismatch`), emitted in either of two cases: (1) Kodi's report never included this setting at all, so `observed` is empty; or (2) the setting's intent was configured but no concrete value was resolved for this run, so `expected` shows the symbolic intent (e.g. `hdmi-multichannel`) rather than a resolved `ALSA:` string — `observed` may still show a real value read from the device in this case. If the owning component was not requested at all, no keys are emitted for the setting and `unobservable` never appears; absence of the key is a different condition from a status of `unobservable`. | Check whether `expected` is a resolved `ALSA:` value or a symbolic intent: if symbolic, resolve the missing concrete value for the intent (case 2). If `expected` is already resolved and `observed` is empty, investigate why Kodi's JSON-RPC report omitted the setting (case 1). |
